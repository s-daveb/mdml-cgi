...
CgiRequestClient::CgiRequestClient(
    ...
    : Application(argc, argv, env), server_socket()
{
}
CgiRequestClient::~CgiRequestClient() {}

Result<std::string>
CgiRequestClient::ProcessRequest()
{
	...
	try {
		...
		buffer.clear();
		buffer << environment.at("REQUEST_URI") << ";"
		       << environment_blob << std::flush;
...
inline void
throw_socket_error(const std::string& msg)
{
	error_buffer.clear();
...
void
CgiRequestClient::connect_to_server()
{
...
std::string
CgiRequestClient::query_server(const std::string& query)
{
	auto& sockfd = this->server_socket;
