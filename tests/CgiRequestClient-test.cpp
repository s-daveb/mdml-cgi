...
struct test_server {
	...
	strcpy(socket_addr.sun_path, TEST_SOCKET_PATH);

	// strcpy(socket_addr.sun_path, TEST_SOCKET_PATH);
	unlink(socket_addr.sun_path);
	if (::bind(server_socket, (sockaddr *)&socket_addr,
...
BEGIN_TEST_SUITE("CgiRequestClient") {
    using namespace mdml;
    struct TestFixture {
	const char *envp[4] = {"PATH=/usr/bin", "VAR2=TWO",
	                       "REQUEST_URI=markdown?msg=hello-world", nullptr};
	...
	CgiRequestClient test(1, simulated_launch::argv, simulated_launch::env);
    };
	...
	REQUIRE_THROWS_AS([&]() { test.ProcessRequest(); }(), mdml::exception);
    }
